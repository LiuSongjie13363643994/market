...
@interface LPCollectionViewLayout ()
/** 存放所有cell的布局属性 */
...
@implementation LPCollectionViewLayout
#pragma mark -- 代理数据处理
/** 行间隔 */
- (CGFloat)rowInterval  {
...
- (void)prepareLayout {
    ...
    attrs = [self layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionFooter atIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
    [self.attrsArray addObject:attrs];
}
//决定cell的布局
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([UICollectionElementKindSectionFooter isEqualToString:elementKind]){
        ...
        attrs.frame=CGRectMake(0,h,self.footerReferenceSize.width,self.footerReferenceSize.height);
    } else {
...
- (CGSize)collectionViewContentSize {
    CGFloat h=self.contentHeight + [self edgeInsets].bottom+self.footerReferenceSize.height;
