...
- (BOOL)invokeWithInfo:(id<AspectInfo>)info {
    ...
    //获得返回值类型
    const char *returnType=self.blockSignature.methodReturnType;
    //声明返回值变量
    ...
    if( !strcmp(returnType, @encode(void)) ){
        ...
        if (0==strcmp(returnType,@encode(id))||0==strcmp(returnType,@encode(Class))) {
            __autoreleasing id returnObj;
            [blockInvocation getReturnValue:&returnObj];
            self.returnValue=returnObj;
        } else {
            self.returnValue=nil;
        }
    }
